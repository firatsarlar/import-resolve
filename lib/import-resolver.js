...
// an extra  testing parameter for unit tests
ImportResolver.prototype.resolve = function(oldFile, testing){
    
    const commentRegex = /((?:\/\*(?:.|\s)*?\*\/)|(?:\/\/.*))/g;

    // remove comments and make a copy of oldFile to keep comments later
    let oldFileCopy = oldFile.replace(commentRegex, '');
    ...
    // funny but just for unit testing .. 
    // return matched imports
    if (testing)  return matches
